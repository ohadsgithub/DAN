...
int main(int argc, char** argv) {
    ...
    for (y = 0; y < image_height; ++y) {
        for (x = 0; x < image_width; ++x) {
            data_padded[x+y*padded_width]   = data[3*(x+y*image_width)];
            data_padded[x+y*padded_width+padded_height*padded_width]   = data[3*(x+y*image_width)+1];
            data_padded[x+y*padded_width+2*padded_height*padded_width]   = data[3*(x+y*image_width)+2]; //is there a need to divide by 255?
        }
    }
        ...
        for (i = 0; i < w_blocks; ++i) {
            ...
            for (y = 0; y < 255; ++y) {
                for (x = 0; x < 255; ++x) {
                    ...
                    patch_input_data[3*(x+y*255)]   = data_padded[3*(x2+y2*padded_width)+1];
                    patch_input_data[3*(x+y*255)]   = data_padded[3*(x2+y2*padded_width)+2];
                }
            }
