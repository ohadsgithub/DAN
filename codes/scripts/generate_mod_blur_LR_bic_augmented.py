...
def generate_mod_LR_bic_augmented():
    # set parameters
    up_scale = 2  # was 4
    mod_scale = 2  # was 4
    # set data dir
    ...
    sourcedir = "C:/tapMobileProj/dataset/data_resized_side/x3/source"  # why /? was "/data/Set5/source/"
    savedir = "C:/tapMobileProj/dataset/data_resized_side/x3"  # was "/data/Set5/"    extra / ?
    ...
    saveKernelpath = os.path.join(savedir, "Kernels", "x" + str(up_scale))
    ...
    if not os.path.isdir(os.path.join(savedir, "Kernels")):
        os.mkdir(os.path.join(savedir, "Kernels"))
    ...
    for i in range(num_files):
        ...
        for sig in np.linspace(1.8, 3.2, 8):
            ...
            #torch.save(ker_map, os.path.join(kernelPath, 'sig{}_{}'.format(sig, filename)))
            tempstring=('sig{}_{}'.format(sig, filename)).replace(".", "p")
            torch.save(ker_map, os.path.join(saveKernelpath, tempstring[:-4]+".pth"))
        # LR
